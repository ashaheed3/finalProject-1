...
class MyRecipeCard extends Component {
  ...
  componentDidMount() {
    SpoonacularAPI.search(this.props.id)
      .then((res) =>
        this.setState({ ingredientResults: res.data.extendedIngredients })
      )
      ...
      case false:
        API.saveFavorites({
          ...
          videoID: this.state.youtubeVideo
            ? this.state.youtubeVideo.id.videoId
            : 'novid',
  ...
  renderIngredients = () => {
    if (!this.state.ingredientResults.length) {
      ...
      return this.state.ingredientResults.map((ingredient) => {
        return <Ingredients name={ingredient.name} key={ingredient.id} />;
      });
    }
  };
  ...
  renderYoutubeVideo = () => {
    if (!Object.keys(this.state.youtubeVideo).length) {
      return <Modal id='modal' title='Video unavailable' videoId='' />;
    } else {
      return (
        <Modal
          id='modal'
  ...
  render() {
    ...
    return (
      <div
        ...
        style={{ backgrounColor: 'antiquewhite' }}
      >
        <img
          className='recipeImg'
          src={'https://spoonacular.com/recipeImages/' + this.props.thumbnail}
        ...
        <div className='card-body'>
          ...
          <div className='d-flex justify-content-between align-items-center'>
            <button
              type='button'
              className='btn btn-md btn-outline-secondary viewRecipeBtn'
            >
